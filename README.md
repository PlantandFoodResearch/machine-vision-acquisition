...
1. Create `config.json` in input day folder. Create in first day for multi-day scans. See
`/input/projects/dhs/smartsensingandimaging/development/fops/2022-12-06/config.json` for
example. Set the side_assignment either `by_run` or `by_head` (camera head).
1. Set `days` list in `extract_metadata.ipynb` cell 1. Optionally set cameras string to
match against camera names, or "*" for all cameras.
1. Run all cells. Check camera position plots have expected coverage.
1. Run `export_day` pipeline:

``` sh
cd pipelines/export_day
nextflow run . --day <YYYY-MM-DD> -bg --tonemap --out_ext jpg
```

`export_day` runs `mva_process convert` on a folder of images containing
`metadata_last.csv`, generated by `extract_metadata.ipynb`, then shares to CloudStor.
