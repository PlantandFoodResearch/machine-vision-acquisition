...
def liveview_web(cameras: List[CameraHelper]):
    ...
    def video_feed_1():
        return Response(
            gen_frames(cameras[0]), mimetype="multipart/x-mixed-replace; boundary=frame"
        )

    @app.route("/video_feed_2")
    def video_feed_2():
        return Response(
            gen_frames(cameras[1]), mimetype="multipart/x-mixed-replace; boundary=frame"
        )

    @app.route("/video_feed_3")
    def video_feed_3():
        return Response(
            gen_frames(cameras[2]), mimetype="multipart/x-mixed-replace; boundary=frame"
        )

    @app.route("/video_feed_4")
    def video_feed_4():
        return Response(
            gen_frames(cameras[3]), mimetype="multipart/x-mixed-replace; boundary=frame"
        )
