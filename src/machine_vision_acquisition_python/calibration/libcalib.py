...
def load_from_calibio_json(calibio_json: Path) -> List[Calibration]:
    ...
    for camera in calib["Calibration"]["cameras"]:
        try:
            polymorphic_name = camera["model"].get("polymorphic_name") or polymorphic_name  # preserve previous model if new one is None
            ...
            if polymorphic_name is None and polymorphic_id != 1:
                ...
                raise NotImplementedError("Cannot support differing camera models in single calibration (yet!)")
            else:
    ...
    for camera in valid_calibs:
        ...
        calib = Calibration(serial, cam_matrix, distortion_matrix, rvec, tvec, image_size["width"], image_size["height"], camera_model)
        # Use openCV naming
